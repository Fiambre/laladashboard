...
type webrtcServer interface {
	ServeWebRTC(w http.ResponseWriter, r *http.Request, inst widgets.WidgetInstance)
}

func WidgetWebRTC(store *config.Store, reg *registry.Registry) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		for _, inst := range cfg.Widgets {
			...
			ws, ok := widget.(webrtcServer)
			if !ok {
				http.Error(w, "widget does not support WebRTC", http.StatusNotFound)
				return
			}
			inst.DataDir = filepath.Join("data", "widgets", inst.ID)
			os.MkdirAll(inst.DataDir, 0755) //nolint:errcheck
			ws.ServeWebRTC(w, r, inst)
			return
		}
		http.NotFound(w, r)
	}
